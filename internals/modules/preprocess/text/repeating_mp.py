...
def run_repeating(string):
    ...
    rstring = ' '.join([word for word in string.split(' ') if word not in repeating_characters])
...
def processr(*args):
    ...
    for word in replace_short.iterrows():
        replace_original.loc[replace_original.short == word[1][0], 'changed'] = True
        replace_original.loc[replace_original.short == word[1][0], 'short'] = word[1][2]
    ...
    replace_original = replace_original[replace_original['changed'] == True].drop(columns='changed').set_index('word').to_dict()['short']
