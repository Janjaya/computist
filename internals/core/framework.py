...
class Framework(cmd.Cmd):
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        if cmds:
            ...
            for _cmd in cmds:
                self.stdout.write(f"{_cmd.ljust(15)} {getattr(self, 'do_' + _cmd).__doc__}{os.linesep}")
    ...
    def table(self, data, header=[], title=""):
        ...
        for i in range(0, cols):
            lens.append(len(max([self.to_unicode_str(x[i]) if x[i] is not None else "" for x in tdata], key=len)))
        ...
        if len(tdata) > 0:
            ...
            if header:
                ...
                print(separator)
            for rdata in tdata:
                data_sub = tuple([self.to_unicode_str(rdata[i]).ljust(lens[i]) if rdata[i] is not None else "".ljust(lens[i]) for i in range(0, cols)])
                print(data_str % data_sub)
    ...
    def get_tables(self):
        return [x[0] for x in self.query("SELECT name FROM sqlite_master WHERE type='table'") if x[0] not in ["dashboard", "snapshots"]]
    ...
    def insert_snapshot(self, snapshot=None, notes=None, revision=None, mute=False):
        """Adds a snapshot to the database"""
        data = dict(
            snapshot=f"snapshot_{snapshot}",
            date=datetime.strptime(str(snapshot), "%Y%m%d%H%M%S").strftime(self.time_format),
            revision=revision,
            notes=notes
        )
    ...
    def save_dataframe(self):
        ...
        if first:
            ...
            self.insert_snapshot(snapshot=f"{ts}", notes=message, revision=cr, mute=True)
    ...
    def show_snapshots(self):
        ...
        if snapshots:
            self.table([list(s) for s in snapshots], header=["Snapshots", "Dates", "Notes", "Modules"])
        else:
