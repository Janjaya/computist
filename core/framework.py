...
class Framework(cmd.Cmd):
    ...
    def __init__(self, params):
        ...
        self._revisioning = True if sys.modules.get('svn.local') else False
        self.pbar = None
        self._exit = 0
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        if cmds:
            ...
            for _cmd in cmds:
                self.stdout.write(
                    (f"{_cmd.ljust(15)} "
                     + f"{getattr(self, 'do_' + _cmd).__doc__}{os.linesep}")
                )
    ...
    def table(self, data, header=[], title=""):
        ...
        for i in range(0, cols):
            lens.append(len(max(
                        [self.to_unicode_str(x[i])
                         if x[i] is not None else "" for x in tdata], key=len
                        )))
    ...
    def get_tables(self):
        ...
        ) if x[0] not in ["dashboard", "snapshots"]]
    ...
    def insert(self, table, data, unique_columns=[]):
        ...
        if not unique_columns:
            ...
            query = (f"INSERT INTO `{table}` (`{columns_str}`) "
                     + f"SELECT {placeholder_str} WHERE NOT EXISTS(SELECT * "
                     + f"FROM `{table}` WHERE {unique_columns_str})")
        values = tuple([data[column]
                       for column in columns] + [data[column]
    ...
    def insert_snapshot(self, snapshot=None, notes=None, revision=None,
                        mute=False):
        """Adds a snapshot to the database"""
        data = dict(
            snapshot=f"snapshot_{snapshot}",
            date=datetime.strptime(
                str(snapshot), "%Y%m%d%H%M%S"
    ...
    def save_dataframe(self):
        ...
        # initiate the local repository only once
        if first and self._revisioning:
            ...
            self.insert_snapshot(snapshot=f"{ts}", notes=message, revision=cr,
                                 mute=True)
    ...
    def revert_dataframe(self):
        """Revert dataframe back to first import"""
        r = svn.local.LocalClient(self.workspace)
        r.update(revision=1)
    ...
    def register_option(self, name, value, required, description):
        self.options.init_option(name=name, value=value, required=required,
    ...
    def _validate_options(self):
        for option in self.options:
            ...
            if not type(self.options[option]) in [bool, int]:
                if (self.options.required[option] is True
    ...
    def _list_options(self, options=None):
        ...
        if options:
            ...
            if key_len < 4:
                key_len = 4
            val_len = len(max(
                [self.to_unicode_str(options[x]) for x in options], key=len
            ))
            if val_len < 13:
                val_len = 13
            print("")
            print(pattern % ("Name".ljust(key_len),
                             "Current Value".ljust(val_len),
                             "Required",
                             "Description"))
            print(pattern % (self.ruler*key_len,
    ...
    def request(self, method, url, **kwargs):
        ...
        kwargs["timeout"] = kwargs.get("timeout") or \
            self._global_options["timeout"]
        # process headers
    ...
    def _do_db_notes(self, params):
        ...
        if table in self.get_tables():
            ...
            for rowid in rowids:
                count += self.query(
                    (f"UPDATE `{table}` SET notes=? WHERE ROWID IS ?",
                     (note, rowid))
                )
    ...
    def _do_spool_start(self, params):
        ...
        if not Framework._spool:
            ...
            if not self._is_writeable(filename):
                ...
                Framework._spool = codecs.open(filename, "ab",
                                               encoding="utf-8")
    ...
    def help_options(self):
        ...
        print(
            (f"{os.linesep}Usage: options "
             + f"<{'|'.join(self._parse_subcommands('options'))}> "
             + f"[...]{os.linesep}")
        )
    ...
    def help_modules(self):
        ...
        print(
            (f"{os.linesep}Usage: modules "
             + f"<{'|'.join(self._parse_subcommands('modules'))}> "
             + f"[...]{os.linesep}")
        )
    ...
    def help_db(self):
        ...
        print(
            (f"{os.linesep}Usage: db "
             + f"<{'|'.join(self._parse_subcommands('db'))}> "
             + f"[...]{os.linesep}")
        )
    ...
    def _help_db_notes(self):
        ...
        print(
            (f"{os.linesep}Usage: db note <table> "
             + f"[<rowid(s)> <note>]{os.linesep}")
        )
        print(
            ("rowid(s) => ',' delimited values or '-' delimited ranges "
             + f"representing rowids{os.linesep}")
        )
    ...
    def _help_db_insert(self):
        ...
        print(
            ("values => '~' delimited string representing column "
             + f"values (exclude rowid, module){os.linesep}")
        )
    ...
    def help_script(self):
        ...
        print(
            (f"{os.linesep}Usage: script "
             + f"<{'|'.join(self._parse_subcommands('script'))}> "
             + f"[...]{os.linesep}")
        )
    ...
    def help_spool(self):
        ...
        print(
            (f"{os.linesep}Usage: spool "
             + f"<{'|'.join(self._parse_subcommands('spool'))}> "
             + f"[...]{os.linesep}")
        )
    ...
    def help_df(self):
        ...
        print(
            (f"{os.linesep}Usage: df "
             + f"<{'|'.join(self._parse_subcommands('df'))}> "
             + f"[...]{os.linesep}")
        )
    ...
    def _complete_script_record(self, text, *ignored):
        return []
    _complete_script_execute = _complete_script_status = \
        _complete_script_stop = _complete_script_record
